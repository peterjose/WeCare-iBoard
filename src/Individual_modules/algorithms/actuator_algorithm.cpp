...
//Feel free to make changes(Add more scenarios. Discuss before deleting/modifying scenarios)
 ...
 //To check if a row/col is full
 //To ram the corresponding cubes
int sensorRowCheck;
int sensorColCheck;
...
void Algorithms(CubeModule_t Cube[], int pixelCount)
{
    for (int i = 0; i < pixelCount; i++)
    {
        if (Cube[i].sensorStatus && (Cube[i].currentAngle == Cube[i].actuatorHighSetting))
        {
            Cube[i].currentAngle = Cube[i].actuatorLowSetting;
        }
        ...
        */
        ...
        - It is reset with every placement
